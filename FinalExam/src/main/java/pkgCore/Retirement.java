...
public class Retirement {
	...
	public double MonthlySavings() {
		...
		double pmt = Retirement.PMT(this.dAnnualReturnWorking/12, this.iYearsToWork*12, 
				Math.round(Retirement.PV(this.dAnnualReturnRetired / 12, 
						this.iYearsRetired * 12, this.dRequiredIncome - this.dMonthlySSI, 
						0, false) * 100.0) / 100.0, 0, false); // <-- this should be fixed to calculate the real pmt
		return pmt;
	}
	...
	public double TotalAmountToSave() {
		...
		double pv = Math.round(Retirement.PV(this.dAnnualReturnRetired / 12, 
				this.iYearsRetired * 12, this.dRequiredIncome - this.dMonthlySSI, 0, false) * 100.0) / 100.0;
